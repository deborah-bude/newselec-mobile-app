...
const BASE_URL = 'https://newselec.pro/'; // site URL
...
const TITLE = 'Newselec'; // homepage title
...
export default class App extends Component<Props> {
    ...
    constructor(props) {
        ...
        this.url = `${BASE_URL}?skin=${TEMPLATE}&mix=${'dark' === colorScheme ? MIX_DARK : MIX_LIGHT}&lang=fr`;
...
*/
    ...
    onHomeMenu() {
        ...
        if (`${BASE_URL}` == this.url || this.url.startsWith(`${BASE_URL}?skin=${TEMPLATE}`) || `${BASE_URL}index.php` == this.url) {        
            ...
            this.setState ({
                url: `${BASE_URL}?skin=${TEMPLATE}&mix=${'dark' == this.state.colorScheme ? MIX_DARK : MIX_LIGHT}&lang=fr`,
                status: '',
                searchbar: false,
    ...
    async componentDidMount() {
        ...
        Appearance.addChangeListener(({ colorScheme }) => { // not working in Android :(
            ...
            if (s != this.state.colorScheme) {
                var sUrl = this.url;
                var sUri = `skin=${TEMPLATE}&mix=${'dark' === s ? MIX_DARK : MIX_LIGHT}&lang=fr`;
...
*/
    ...
    render() {
        var sUri = `${BASE_URL}?skin=${TEMPLATE}&mix=${'dark' === this.state.colorScheme ? MIX_DARK : MIX_LIGHT}&lang=fr`;
